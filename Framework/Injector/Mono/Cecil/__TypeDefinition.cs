...
namespace Mono.Cecil
{
    static internal class __TypeDefinition
    {
            ...
            where T : Attribute
        {
            ...
            foreach (var _argument in (expression.Body as NewExpression).Arguments) { _attribute.ConstructorArguments.Add(new CustomAttributeArgument(type.Module.Import(_argument.Type), Expression.Lambda<Func<object>>(Expression.Convert(_argument, System.Reflection.Metadata<object>.Type)).Compile()())); }
            type.CustomAttributes.Add(_attribute);
            return _attribute;
        }
