...
namespace Mono.Cecil
{
    static internal class __AssemblyDefinition
    {
            ...
            where T : Attribute
        {
            ...
            foreach (var _argument in (expression.Body as NewExpression).Arguments) { _attribute.ConstructorArguments.Add(new CustomAttributeArgument(assembly.MainModule.Import(_argument.Type), Expression.Lambda<Func<object>>(Expression.Convert(_argument, System.Reflection.Metadata<object>.Type)).Compile()())); }
            assembly.CustomAttributes.Add(_attribute);
            return _attribute;
        }
    }
